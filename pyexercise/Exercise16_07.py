...
def main():
    N = 1000 # The number of the points
    points = []
    for i in range(N):
        points.append([random() * 100, random() * 100])
      
    startTime = time.time()
    pair = closestPairBruteForce(points)
    print("shortest distance is", distance(pair))
    print("Two points are", pair)
    endTime = time.time() 
    print("Time spent on the brute-force algorithm is", (endTime - startTime), "milliseconds")
...
def closestPairBruteForce(double[][] points) {
